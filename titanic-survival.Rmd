...
- [Model development](#dev). The key section in specifying, developing, validating and describing a binary logistic model, split into 
  ...
  - [Model fitting and validation](#fit). We fit final pooled model over 30 imputed dataset. Bootstrap internal validation (the ".632" method) is used to study optimism-corrected index measuring discrimination and calibration. 


- [Interpratation and discussion](#interpretation). We describe the model with both graphical methods such as partial effect plots and statistical testing. This provies model-based explanation to address research problems.  
...
We could now address question 1, 2 and 3 with figure \@ref(fig:partial-age-sex-cabin), where we need to inspect all these three variables' influence on survival status. We want to know if women, youth and higher class results in greater chance to survive. First note that the confidence interval is extremely wide for young first class passengers, and for the crew whose age is not in the middle. Thus we tend to only interpret their age effects where the prediction is less variable.  

Women and children are evacuated first in general, supported both by data and stories. `r label_percent(0.1)(mean(subset(t, gender == "Female")$survived == 1))` of the women and `r label_percent(0.1)(mean(subset(t, age <= 12)$survived == 1))` of the children (age under 12) were saved, but only `r label_percent(0.1)(mean(subset(t, gender == "Male")$survived == 1))` of the men. There was also recollection that at the beginning of the sinking,  the Second Officer  on the Titanic suggested to Captain Smith, "Hadn't we better get the women and children into the boats, sir?", to which the captain responded: "put the women and children in and lower away".  Fifth Officer Harold Lowe exposited the policy in the American Inquiry as 

> It was simply the first woman, whether first class,
second class, third class, or sixty-seventh class. It was all the
same; women and children were first

A closer look at numbers may suggest another story. We observe that first class passengers have the highest predicted survival possibility in general, whereas the third class is the lowest. The chasm between classes is most manifest in adult women. A 30-year-old first class women is likely to survive with a probability between (`r label_percent(0.1)(Predict(f2, age = 30, class = "1st", gender = "Female", fun = plogis)$lower)`, `r label_percent(0.1)(Predict(f2, age = 30, class = "1st", gender = "Female", fun = plogis)$upper)`), and if she were in the third class, (`r label_percent(0.1)(Predict(f2, age = 30, class = "3rd", gender = "Female", fun = plogis)$lower)`, `r label_percent(0.1)(Predict(f2, age = 30, class = "3rd", gender = "Female", fun = plogis)$upper)`). 

Interestingly, the crew out of four roles share a similar survival trend with first class passengers.


. The first class are the group of people that survive the most in percentages. And Since they have roughly similar prediction intervals especially in the middle age range, there is not much evidence to argue who is saved first. In fact, there might even be a slight advantage for the middle-aged over the elderly (note the confidence starts to get wider in the end).  The most decisive explanation for such effect is that first-class passengers had better access to information about the
...
```{r, out.width = "90%", fig.cap ="The Titanic's route. Source: https://evansexpertsattheroutethird.weebly.com/route"}
