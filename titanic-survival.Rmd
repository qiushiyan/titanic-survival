...
- [Modeling](#modeling). multiple imputation; partial effects 
...
## Model fitting and penalization 
...
```{r, include = FALSE}
f2 <- fit.mult.impute(survived ~ rcs(age, 5) * gender + class_dept +   nationality + spouse + sibling + parent + children, lrm, imp, data = t)
```


```{r, results="asis"}
mylatex(anova(f2), file = "")
```
