...
```{r, results="asis"}
mylatex(anova(f1), file="", size = "small", table.env = TRUE, caption = "Hypothesis testing for the saturated model", label = "saturated-table")
```

Table \@ref(saturated-table) 
...
attr(v1, "kept") %>% 
  ...
  mylatex(file = "", 
          table.env = TRUE, label = "data-dictionary", size = "small",
          caption = "Terms retained less than 20 times in the model in 200 bootstrap resamples", rowname = NULL, where = "h")
```
...
```{r, results = "asis"}
mylatex(anova(f2), file = "", size = "small")
```
