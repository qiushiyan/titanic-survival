...
Aside from rank-based discrimination index, figure \@ref(fig:cal-curve) aims to gauge the concordance between predicted values and observed data, or in other words, calibration.  The actual probability is estimated with loess regression, and the bias correction is computed in a similar way as in table \@ref(model-index). The 45 degree line indicates the ideal scenario in which prediction perfectly matches observation. The model is well-calibrated by and large, with slight departure from the straight line in lower and certain middle region. The mean squared error and 0.9 quantile of absolute error
is 0.00018 and 0.023, respectively. Similarly, the unreliability index $E_{max} =$ `r round((v2["Emax", "index.corrected"]/2)+0.5, 2)` measures maximum error in predicted probabilities. All these metrics reflects strong calibration. 
...
However, this code of conduct is not respected among third class passengers, where we see two abnormal trends. Firstly, people in their prime have higher predicted survival possibility. The survival curve displays an inverted U-shape, where people aged around  for both men and women. Secondly, the gender gap also narrowed, if someone is a third class miss or elderly woman, it is hard to conclude she has any priority in getting in the lifeboat over a man of similar age. For example, 
